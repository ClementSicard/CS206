...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    val nb = Boundaries()
    nb.minX = math.min(body.x, boundaries.minX)
    nb.minY = math.min(body.y, boundaries.minY)
    nb.maxX = math.max(body.x, boundaries.maxX)
    nb.maxY = math.max(body.y, boundaries.maxY)
    nb
  }
  ...
  def computeSectorMatrix(bodies: coll.Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.aggregate(new SectorMatrix(boundaries,SECTOR_PRECISION))((x,y) => x += y, _.combine(_))
  }
