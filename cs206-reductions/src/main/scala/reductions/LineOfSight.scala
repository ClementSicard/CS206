...
object LineOfSight extends LineOfSightInterface {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit =
    for (i <- input.indices) if i == 0 then output(i) else output(i) = max(input(i) / i, output(i - 1))

  /** Traverses the specified part of the array and returns the maximum angle.
   */
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    (for (i <- from to until - 1) yield if i != 0 then input(i) / i else 0).max
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int, threshold: Int): Tree = {
    val s = end - from
    if s <= threshold then Leaf(from, end, upsweepSequential(input, from, end)) else
      val s2 = s / 2
      val (l, r) = parallel(upsweep(input, from, from + s2, threshold), upsweep(input, from + s2, end, threshold))
      Node(l, r)
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int): Unit = {
    if from < until then
      val m = max(input(from) / from, startingAngle)
      output(from) = m
  ...
  def max(a : Float, b : Float) = if a >= b then a else b
