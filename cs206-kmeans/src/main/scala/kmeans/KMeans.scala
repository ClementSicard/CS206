...
class KMeans extends KMeansInterface {
  ...
  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = 
    val x = points.groupBy(x => findClosest(x, means))
    means.map(i => (i, if x.contains(i) then x(i) else ParSeq.empty)).toMap
  ...
  def update(classified: ParMap[Point, ParSeq[Point]], oldMeans: ParSeq[Point]): ParSeq[Point] = oldMeans.map(x => findAverage(x, classified(x)))

  def converged(eta: Double, oldMeans: ParSeq[Point], newMeans: ParSeq[Point]): Boolean = oldMeans.zip(newMeans).map(x => x._1.squareDistance(x._2) <= eta).foldLeft(true)(_&&_)
  ...
  final def kMeans(points: ParSeq[Point], means: ParSeq[Point], eta: Double): ParSeq[Point] =
    ...
    if !converged(eta, means, updated) then kMeans(points, updated, eta) else updated // your implementation need to be tail recursive
}
